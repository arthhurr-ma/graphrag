...
async def _text_embed_in_memory(
    ...
    log.info(f"Embedding {len(texts)} texts with a total of {input_token_count} input tokens.")
...
async def _text_embed_with_vector_store(
    ...
    while insert_batch_size * i < input.shape[0]:
        ...
        input_token_count = sum([len(text.split()) for text in texts])  # estimate input tokens
